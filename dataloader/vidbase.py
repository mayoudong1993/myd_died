...
from third_party.ext_utils.util_flow import readPFM
from third_party.ext_utils.flowlib import warp_flow
...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        plt.imshow(elem['img'].transpose(1, 2, 0))
        plt.savefig("input.jpg")
        plt.show()
        plt.imshow(elem['mask'])
        plt.savefig("mask.jpg")
        plt.show()
        plt.imshow(elem['flow'][:, :, 0])
        plt.savefig("flow0.jpg")
        plt.show()
        plt.imshow(elem['flow'][:, :, 1])
        plt.savefig("flow1.jpg")
        plt.show()
        plt.imshow(elem['occ'])
        plt.savefig("occ.jpg")
        plt.show()
        ...
        plt.savefig("dp.jpg")
        plt.show()
        plt.imshow(elem['vis2d'])
        plt.savefig("vis2d.jpg")
        plt.show()
        return elem
...
class VidDataset(BaseDataset):
    ...
    def __init__(self, opts, filter_key=None, imglist=None, can_frame=0,
        ...
        if dframe == 1:
            self.flowfwlist = [
                i.replace('JPEGImages', 'FlowFW').replace('.jpg', '.pfm').replace('.png', '.pfm').replace(
                    '%s/' % seqname, '%s/flo-' % seqname) for i in self.imglist]
            self.flowbwlist = [
                i.replace('JPEGImages', 'FlowBW').replace('.jpg', '.pfm').replace('.png', '.pfm').replace(
                    '%s/' % seqname, '%s/flo-' % seqname) for i in self.imglist]
        else:
            self.flowfwlist = [
                i.replace('JPEGImages', 'FlowFW').replace('.jpg', '.pfm').replace('.png', '.pfm').replace(
            ...
            self.flowbwlist = [
                i.replace('JPEGImages', 'FlowBW').replace('.jpg', '.pfm').replace('.png', '.pfm').replace(
                    '%s/' % seqname, '%s/flo-' % (seqname)) for i in self.imglist]
        self.featlist = [i.replace('JPEGImages', 'Densepose').replace('.jpg', '.pfm').replace('.png', '.pfm').replace(
            '%s/' % seqname, '%s/feat-' % seqname) for i in self.imglist]
        self.featlist = ['%s/feat-%05d.pfm' % (i.rsplit('/', 1)[0], int(i.split('feat-')[-1].split('.pfm')[0])) for i in
                         self.featlist]
        self.bboxlist = ['%s/bbox-%05d.txt' % (i.rsplit('/', 1)[0], int(i.split('feat-')[-1].split('.pfm')[0])) for i in
                         self.featlist]
        self.kplist = [
            i.replace('JPEGImages', 'KP').replace('.jpg', '_keypoints.json').replace('.png', '_keypoints.json') for i in
            self.imglist]
        self.dplist = [i.replace('JPEGImages', 'Densepose').replace('.jpg', '.pfm').replace('.png', '.pfm') for i in
                       self.imglist]
        ...
        self.baselist = [i for i in range(len(self.imglist) - self.dframe)] + [i + self.dframe for i in
                                                                               range(len(self.imglist) - self.dframe)]
...
if __name__ == '__main__':
    opt = {'n_data_workers': 1, 'rtk_path': None, 'dframe': [1], 'can_frame': -1,
           'init_frame': 0, 'end_frame': -1, 'seqname': 'rs_dog', 'ngpu': 1,
           'batch_size': 2, 'datapath': '/data/ji/BADJA/JPEGImages/Full-Resolution/rs_dog/', 'img_size': 512,
           'local_rank': 0, 'preload': False, 'accu_steps': 1}
    imglist = sorted(glob.glob('%s/*' % opt['datapath']))
    ds = VidDataset(opt, filter_key=None, imglist=imglist)
    data = ds[0]
    print('hello')